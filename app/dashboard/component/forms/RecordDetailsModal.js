...
function VarietyRow({ row, index, cropName, usedNames, onChange, onRemove }) {
  ...
  const totalProduction =
    (parseFloat(row.achievement) || 0) * (parseFloat(row.productionPerHa) || 0);
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <input
            type="number"
            value={row.productionPerHa || ""}
            min={0}
            step={0.01}
            placeholder="0.00"
            aria-label="Production per hectare in metric tons"
            onChange={(e) =>
              onChange(
                index,
                "productionPerHa",
        ...
        {/* Total production — auto-calculated, read-only */}
        <div>
          <div
            style={{
              fontSize: 10,
          ...
          <div
            style={{
              ...
              background: "#f0f0f0",
              color: totalProduction > 0 ? "#0F6E56" : "#bbb",
              fontWeight: totalProduction > 0 ? 500 : 400,
            }}
          >
...
//     (s, r) =>
//       s +
//       (parseFloat(r.achievement) || 0) * (parseFloat(r.productionPerHa) || 0),
//     0,
//   );
//   const pct = item.target ? Math.round((total / item.target) * 100) : 0;
...
//       production_total: parseFloat(totalProduction.toFixed(2)),
...
//       onSave(item.id, rows, total, totalProduction);
//     } catch {
//       // demo fallback
//       onSave(item.id, rows, total, totalProduction);
//     } finally {
...
//                 </span>
//                 <span
//                   style={{ fontSize: 15, fontWeight: 500, color: "#0F6E56" }}
//                 >
//                   {totalProduction > 0
...
const RecordDetailsModal = ({
  ...
  const handleSave = (id, rows, total, production) => {
    setTableData((prev) =>
      prev.map((item) =>
        item.id === id
          ? {
